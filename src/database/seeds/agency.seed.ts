...
export async function up(query: QueryBuilder<object>): Promise<AgencyEntity[]> {
	const agencies = await query
		.select("agency")
		.from(AgencyEntity, "agency")
		.getMany();
...
export async function down(dataSource: DataSource) {
	const tableName = dataSource.getMetadata(AgencyEntity).tableName;
