...
export class CreatePremisesDto {
    ...
    @ApiProperty({ description: 'Status of the commercial premise', enum: CommercialStatus, required: false })
    ...
    @ApiProperty({ description: 'Photos URLs', type: [String], required: false })
    @IsArray()
    @IsString({ each: true })
    @IsOptional()
    photos?: string[];

    @ApiProperty({ description: 'Number of similar apartments', required: false })
    @IsNumber()
    @IsOptional()
